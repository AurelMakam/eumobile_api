...
// Getting the partner code and MPIN
     ...
     * 
     */
    $partnerCodeAndMpin = getCommissionAccount($partnerId, $connection);
    $date = date("Y-m-d", time());
    $time = date("H:i:s", time());
    if (is_array($partnerCodeAndMpin)) {
        $partnerCode = $partnerCodeAndMpin["account"];
        $partnerPin = $partnerCodeAndMpin["mpin"];
        try {
...
//                AddTransaction($partnerId, "",$serviceId,"BALANCE", $partnerCode, "", $result["balance"], "","", $date." ".$time, -1,$langFront["Label"][18] , $connection);
                    ...
                    AddTransaction($partnerId, "", $serviceId, "COMMISSION BALANCE", $partnerCode, "", $result["balance"], "", "", $date . " " . $time, 0, $langFront["Label"][7], "", null, null, 1, $connection);
                    echo json_encode(array("statut" => 100, "message" => $langFront["Label"][7], "account" => $partnerCode, "balance" => $result["balance"]));
                }
            } else {

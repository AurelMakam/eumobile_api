                ...
                if ($result = get_web_page($url)) {
                    ...
                    if (is_array($tab) && count($tab) > 0) {
                        ...
                        if (!is_null($transdetails) && $transdetails != -1) {
                            ...
                            AddTransaction($partnerId, $transaction, $serviceId, "PURCHASE", $partnerCode, $transdetails["destination"], $transdetails["amount"], $transdetails["fee"], $transdetails["tax"], $transdetails["date"], $transdetails["result_code"], $transdetails["result_desc"], $transaction, null, $commission, 0, $connection);
                            echo json_encode(array("statut" => 100, "message" => $transdetails["result_desc"], "amount" => $transdetails["amount"], "fees" => ($transdetails["fee"] + $transdetails["tax"]), "merchant" => $merchant, "transaction" => $transdetails["trans_id"], "reference" => $transdetails["reference"], "balance" => $partnerdetails["balance"], "datetime" => $transdetails["date"], "commission" => $commission));
                        } else {
                            saveRequest($serviceId, $partnerId, $m_hash, $amount, 1, $connection);
                            AddTransaction($partnerId, $transaction, $serviceId, "PURCHASE", $partnerCode, $phone, $tab[0], "", "", $date . " " . $time, 0, $langFront["Label"][7], $transaction, null, null, 0, $connection);

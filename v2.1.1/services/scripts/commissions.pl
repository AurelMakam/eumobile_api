...
if($prep->rows()>0) {
    ...
    while(my @val1 = $prep->fetchrow_array) {
        ...
        my $req1 = "SELECT * FROM tb_transactions WHERE col_partner_id = '".$partnerId."' AND col_result_desc = 'Transaction Successful' AND col_status='0' ORDER BY col_id ASC LIMIT 1";
...
#                ($bdd->prepare("UPDATE tb_transactions SET col_status = '1' WHERE col_transaction_id='".$trxn."'"))->execute();
        ...
        if($commission_amount>0) {
            print $write_log "Total commission calculated for partner id ".$partnerId." = ".$commission_amount." for transaction $transaction_list \n";
        ...
        [
            ...
            if ($status eq "100") {
                ...
                print $write_log "successfully credited commission of partner number ".$partnerId." to total amount : $commission_amount for transaction $transaction_list\n";
                my $query = "UPDATE tb_transactions SET col_status = '1', col_commission='".$commission_amount."' WHERE col_transaction_id IN (".$transaction_list.")"; 
                ($bdd->prepare($query))->execute();
                my $query2 = "UPDATE  tb_commissions_transfer SET col_result_desc = '".$transaction_list."' WHERE col_transaction_id = '".$txn."'"; 
                ($bdd->prepare($query2))->execute();
            }
            else {
			...
			if($transaction_list ne ""){
				if ((substr $transaction_list, -1) eq ","){chop $transaction_list;}
				my $query2 = "UPDATE tb_transactions SET col_status = '1' WHERE col_transaction_id IN (".$transaction_list.") AND col_partner_id = '".$partnerId."'"; 
                ($bdd->prepare($query2))->execute();
			}

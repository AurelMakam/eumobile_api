...
//            sauvegarde du résultat dans les logs
                    ...
                    if (is_array($tab) && count($tab) > 0) {
                        ...
                        if (!is_null($transdetails) && $transdetails != -1) {
                            ...
                            AddTransaction($partnerId, $transactionId, $serviceId, "RECEIVE MONEY", $partnerCode, $d_phone, $transdetails["amount"], $transdetails["fee"], $transdetails["tax"], $transdetails["date"], $transdetails["result_code"], $transdetails["result_desc"],$holdacc["trans_id"],0, $connection);
                            echo json_encode(array("statut" => 100, "message" => $langFront["Label"][7], "amount" => $transdetails["amount"], "fees" => $transdetails["fee"]+$transdetails["tax"], "transaction" => $transactionId, "receiver_phone" => $d_phone, "balance" => $partnerdetails["balance"]));
                        } else {
                            managerLogSimple(__FILE__, __CLASS__, $ip, $partnerId, $service, "decharge effectuee avec succees pour le numero " . $d_phone . " par " . $partnerCode . " montant " . $amount . " XAF");
                            saveRequest($serviceId, $partnerId, $m_hash, $amount, 1, $connection);
                            saveRecvMoney($partnerId, $d_phone, $amountrecv, $fees, $cnitype, $cni, $d_name, $s_name, $s_phone, $date, $transactionId, 1, $connection);
                            AddTransaction($partnerId, $transactionId, $serviceId, "RECEIVE MONEY", $partnerCode, $d_phone, $amountrecv, $fees, "", $date, 0, $langFront["Label"][7],$holdacc["trans_id"],0, $connection);
...
//{"statut":100,"message":"Transaction Successful","amount":"10000.0000","fees":"0.0000","transaction":"9548492","receiver_phone":"237691876792","balance":"87200.0000"}
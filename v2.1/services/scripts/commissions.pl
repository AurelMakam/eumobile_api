...
if($prep->rows()>0) {
    ...
    while(my @val1 = $prep->fetchrow_array) {
        ...
        my $req1 = "SELECT * FROM tb_transactions WHERE col_partner_id = '".$partnerId."' AND col_result_desc = 'Transaction Successful' AND col_status='0' LIMIT 0,1";
...
#                ($bdd->prepare("UPDATE tb_transactions SET col_status = '1' WHERE col_transaction_id='".$trxn."'"))->execute();
        ...
        print $write_log "Total commission calculated for partner id ".$partnerId." = ".$commission_amount." for transactions ".$transaction_list." \n";
        ...
        if($commission_amount>0) {
            ...
            $md5_class->add($partnerId.$walletId.$commission_amount.$transaction_list.$commission_key);

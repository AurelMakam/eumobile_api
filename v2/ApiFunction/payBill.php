...
//            sauvegarde du résultat dans les logs
                        ...
                        if (is_array($tab) && count($tab) > 0) {
                            ...
                            if(!is_null($transactiondetails) && $transactiondetails != -1){
                                AddTransaction($partnerId, $transaction,$serviceId, "BILLPAY", $partnerCode, $billercode, $transactiondetails["amount"], $transactiondetails["fee"], $transactiondetails["tax"], $transactiondetails["date"], $transactiondetails["result_code"], $transactiondetails["result_desc"],$transaction,0, $connection);
                                saveRequest($serviceId, $partnerId, $m_hash, $amount, 1, $connection);
                                echo json_encode(array("statut" => 100, "message" => $langFront["Label"][7], "billno" => $billno, "biller" => $biller, "amount" => $transactiondetails["amount"], "fees"=>($transactiondetails["fee"] + $transactiondetails["tax"]), "transaction" => $transactiondetails["trans_id"], "balance"=>$partnerdetails["balance"]));
                            }
                            else{
                                AddTransaction($partnerId, $transaction,$serviceId, "BILLPAY", $partnerCode, $billercode, $amount, "", "", $date." ".$time, 0, $langFront["Label"][7], $transaction,0, $connection);
                                saveRequest($serviceId, $partnerId, $m_hash, $amount, 1, $connection);

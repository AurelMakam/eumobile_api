 ...
 * @return boolean
 */
function AddTransaction($partnerId, $trans_id, $serviceId, $type, $source, $dest, $amount, $fees, $tax, $datetime, $result_code, $result_desc, $comment, $status, $dbConnexionID) {
    $sqlQuery = "INSERT INTO tb_transactions(col_transaction_id,col_partner_id,col_service_id, col_type,col_source,col_destination,col_amount,col_fees,col_tax,col_datetime,col_result_code,col_result_desc,col_comments, col_status) VALUES ('" . $trans_id . "','" . $partnerId . "','" . $serviceId . "','" . $type . "','" . $source . "','" . $dest . "','" . $amount . "','" . $fees . "','" . $tax . "','" . $datetime . "','" . $result_code . "','" . $result_desc . "', '" . $comment . "', '" . $status . "')";
...
function SubsDetails($phone, $informixConnection) {
    ...
    try {
        ...
        if (strcmp($res['MBA_ABBR'], $phone) == 0) {
            return array("phone" => $res['MBA_ABBR'], "name" => $res['MBA_NAME'], "statut" => getStatus($res['MBA_STATUS_ID']), "plan" => $res['MBSPL_ABBR'], "balance" => $res['MBW_VALUE']);
        } else {
            return null;
        }
    } catch (Exception $ex) {
...
function TransDetails($transid, $informixConnection) {
    $Query = "SELECT mb_transaction.mbt_serial_id,mb_transaction.mbt_src_mobile_no,mb_transaction.mbt_dest_mobile_no,mb_transaction.mbt_value,mb_transaction.mbt_tax,mb_transaction.mbt_fee,mb_transaction.mbt_transaction_cts,mb_transaction.mbt_result_code,mb_transaction.mbt_result_desc,mb_transaction.mbt_comments, mb_trans_type.mbtt_name from mb_transaction INNER JOIN mb_trans_type ON mb_trans_type.mbtt_id=mb_transaction.mbt_trans_type_id WHERE mbt_serial_id = '" . $transid . "'";
    try {
        $stmt = $informixConnection->query($Query);
        $res = $stmt->fetch(PDO::FETCH_BOTH);
...
function HoldingAccDetails($transid, $informixConnection) {
    $Query = "SELECT mbha_trans_id,mbha_sender_mobile,mbha_recipient_mobile,mbha_recipient_amount, mbha_agent_fee, mbha_cts, mbha_trans_code FROM mb_holding_acc where mbha_trans_id = '" . $transid . "' ";
    try {
        $stmt = $informixConnection->query($Query);
        $res = $stmt->fetch(PDO::FETCH_BOTH);

        ...
        if (is_null($result)) {
			...
			if(strcmp($result["type"],"SEND MONEY")==0){
				...
				if(!is_null($holdaccount) && $holdaccount != -1){
					echo json_encode(array("statut" => 100, "reference_id"=>$result["reference_id"], "trans_id" => $result["trans_id"], "source"=>$result["source"],"destination"=>$result["destination"],"amount"=>$result["amount"],"fee"=>$result["fee"],"tax"=>$result["tax"],"date"=>$result["date"],"result_code"=>$result["result_code"],"result_desc"=>$result["result_desc"],"type"=>$result["type"], "commission"=>$result["commissions"], "payment_status"=>'pending'));
					exit;
				}
				else{
					if($result["result_code"] == 0){
						...
						echo json_encode(array("statut" => 100, "reference_id"=>$result["reference_id"], "trans_id" => $result["trans_id"], "source"=>$result["source"],"destination"=>$result["destination"],"amount"=>$result["amount"],"fee"=>$result["fee"],"tax"=>$result["tax"],"date"=>$result["date"],"result_code"=>$result["result_code"],"result_desc"=>$result["result_desc"],"type"=>$result["type"],"commission"=>$result["commissions"], "payment_status"=>'declined'));
						exit;
					}
				}
			}
			elseif(strcmp($result["type"],"CASH IN")==0){
				if($result["result_code"] == 0){
					echo json_encode(array("statut" => 100, "reference_id" => $result["reference_id"], "source"=>$result["source"],"destination"=>$result["destination"],"amount"=>$result["amount"],"fee"=>$result["fee"],"tax"=>$result["tax"],"date"=>$result["date"],"result_desc"=>$result["result_desc"],"type"=>$result["type"], "trans_id"=>$result["trans_id"],"commission"=>$result["commissions"],"payment_status"=>'paid'));
					exit;
				}
				else{
					echo json_encode(array("statut" => 100, "reference_id" => $result["reference_id"], "source"=>$result["source"],"destination"=>$result["destination"],"amount"=>$result["amount"],"fee"=>$result["fee"],"tax"=>$result["tax"],"date"=>$result["date"],"result_desc"=>$result["result_desc"],"type"=>$result["type"], "trans_id"=>$result["trans_id"],"commission"=>$result["commissions"],"payment_status"=>'declined'));

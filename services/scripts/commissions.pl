...
if($prep->rows()>0) {
    ...
    while(my @val1 = $prep->fetchrow_array) {
        ...
        print $write_log "Total commission calculated for partner id ".$partnerId." = ".$commission_amount."  for transaction \n";
        ...
        [
            ...
            if ($status eq "100") {
                ...
                #            my $txn = $message-> {'transaction'};
#            my $fe = $message-> {'fee'};
#            my $ta = $message-> {'tax'};
#            my $date = $message-> {'datetime'};
#            my $sq = "INSERT INTO tb_commission_transfert(col_transaction_id,col_partner_id,col_source,col_destination,col_amount,col_fees,col_tax,col_datetime,col_result_code,col_result_desc) VALUES ('".$txn."','".$partnerId."','".."') ";
            }
            else {

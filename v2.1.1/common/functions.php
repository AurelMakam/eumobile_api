    ...
    try {
        ...
        if (strcmp($res['MBA_ABBR'], $phone) == 0) {
            ...
            foreach($tab as $a_name){
                if(strlen($a_name) > 3){$a_name = substr($a_name, 0,3)."***"; }else{$a_name = substr($a_name, 0,1)."***";}
                $name .= $a_name. " ";
            }
 ...
 * @return int
 */
function addCashInDetails($trans_id, $s_name, $s_country, $d_phone, $d_name ,$amount, $date, $dbConnexionID) {
    $sqlQuery = "INSERT INTO tb_cashin(col_transaction_id,col_sendername,col_sendercountry,col_destphone,col_destname, col_amount,col_date) VALUES ('" . $trans_id . "','" . $s_name . "','" . $s_country . "','" . $d_phone . "','" . $d_name . "', '".$amount."', '" . $date . "')";
    try {
        mysqli_query($dbConnexionID, $sqlQuery);
        return TRUE;
    } catch (Exception $ex) {
 ...
 * @return null
 */
function PartnerDetails($partnerID, $dbConnexionID) {
    $Query = "SELECT * FROM tb_partner WHERE col_partner_id = '" . $partnerID . "' ";
    try {
        $req = mysqli_query($dbConnexionID, $Query);
        if (mysqli_num_rows($req) == 0) {
            return null;
            ...
            $tab = mysqli_fetch_assoc($req);
            return array("partner_id" => $tab['col_partner_id'], "partner_name" => $tab['col_name']);
        }
    } catch (Exception $ex) {
        managerLogDB(__FILE__, __CLASS__, __FUNCTION__, __LINE__, $Query, $ex->getMessage());
        return -1;
...
/**
 * checks wether system can apply KYC for cashIn service
 * @param type $partnerID
 * @param type $dbConnexionID
 * @return boolean
 */
 ...
 * @param type $date_transaction
 * @param type $transaction_reload
 * @param type $date_transaction_reload
 * @param type $description
 * @param type $status
 * @param type $dbConnexionID
 * @return boolean
 */
function AddClaim($partnerId, $phone, $name, $amount, $transaction,$date_transaction, $transaction_reload,$date_transaction_reload, $description, $status, $dbConnexionID) {
    $sqlQuery = "INSERT INTO tb_reclammations(col_partner_id,col_phone,col_name, col_amount,col_transaction, col_date_transaction,col_transaction_reload, col_date_transaction_reload, col_description,col_status) VALUES ('" . $partnerId . "','" . $phone . "','" . $name . "','" . $amount . "','" . $transaction . "','" . $date_transaction . "','" . $transaction_reload . "','" . $date_transaction_reload . "','" . $description . "','" . $status . "')";
 ...
 * @return boolean
 */
function upDateClaim($transaction, $transaction_reload, $date_transaction_reload, $description, $status,$dbConnexionID) {
    $sqlQuery = "UPDATE tb_reclammations SET col_transaction_reload = '".$transaction_reload."', col_date_transaction_reload  = '".$date_transaction_reload."',  col_description = '".$description."', col_status = '".$status."' WHERE col_transaction = '".$transaction."'";
 ...
 * @return type
 */
function getClaim($transaction, $dbConnexionID) {
    
    $sqlQuery = "SELECT * FROM tb_reclammations WHERE col_transaction = '" . $transaction . "'";
    try {
        $resultSqlQuery = mysqli_query($dbConnexionID, $sqlQuery);
        if (mysqli_num_rows($resultSqlQuery) == 0) {
            return null;
...
//            echo "here";
            $dataRow = mysqli_fetch_assoc($resultSqlQuery);
            return array("transaction"=>$dataRow['col_transaction'], "status" =>$dataRow['col_status'] , "amount" => $dataRow['col_amount'] , "phone" => $dataRow['col_phone'] , "description" => $dataRow['col_description']);
        }
    } catch (Exception $ex) {
        managerLogDB(__FILE__, __CLASS__, __FUNCTION__, __LINE__, $sqlQuery, $ex->getMessage());
        return null;
 ...
 * @return type
 */
function getSubaccount($partnerId, $dbConnexionID) {
    
    $sqlQuery = "SELECT col_subaccount, col_subaccountmpin FROM tb_partner WHERE col_id = '" . $partnerId . "'";
    try {
        $resultSqlQuery = mysqli_query($dbConnexionID, $sqlQuery);
        if (mysqli_num_rows($resultSqlQuery) == 0) {
            return null;

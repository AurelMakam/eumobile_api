...
//                    récuperation du timeout du partenaire
                        SavePaymentRequestID($partnerId, $tab[3], $tab[1], $dbConnexionID);
                        ...
                        if ($timeout == 0) {
                            ...
                            $statutr = 0;
                            $inc = 0;
                            while ($statutr == 0 && $inc <= $timeout) {
                                $statutr = getBillStatus($partnerId, $reference, $phone, $connection);
                                if ($statutr == "1") {
                                    saveRequest($serviceId, $partnerId, $m_hash, $amount, 1, $connection);
                                    echo json_encode(array("statut" => 100, "message" => $langFront["Label"][12], "phone" => $tab[0], "amount" => $tab[2], "reference" => $tab[3]));
                                    exit();
                                } else {
                                    $inc++;
                            ...
                            $statutr = 0;
                            $inc = 0;
                            while ($statutr == 0 && $inc <= PAYMENT_TIMEOUT) {
                                $statutr = getBillStatus($partnerId, $reference, $phone, $connection);
                                if ($statutr == "1") {
                                    saveRequest($serviceId, $partnerId, $m_hash, $amount, 1, $connection);
                                    echo json_encode(array("statut" => 100, "message" => $langFront["Label"][12], "phone" => $tab[0], "amount" => $tab[2], "reference" => $tab[3]));

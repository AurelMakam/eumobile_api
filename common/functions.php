...
function SubsDetails($phone, $informixConnection) {
    $Query = "SELECT mb_agent.* , mb_service_plans.mbspl_abbr from mb_agent inner join mb_service_plans on mb_service_plans.mbspl_id=mb_agent.mba_plan_id WHERE mba_abbr = '" . $phone . "'";
    try {
        $stmt = $informixConnection->query($Query);
        $res = $stmt->fetch(PDO::FETCH_BOTH);
        
        if(strcmp($res['MBA_ABBR'], $phone)==0){            
            return array("phone"=>$res['MBA_ABBR'],"name"=>$res['MBA_NAME'],"statut"=>getStatus($res['MBA_STATUS_ID']),"plan"=>$res['MBSPL_ABBR']);
        }
        else{
            return null;
        }
...
function BillDetails($billno, $billercode, $informixConnection) {
    ...
    try {
        ...
        if(strcmp($res['MBSAB_BILL_NO'], $billno)==0){
            return array("billno"=>$res['MBSAB_BILL_NO'],"custid"=>$res['MBSAB_CUST_ID'],"custname"=>$res['MBSAB_CUST_NAME'],"amount"=>$res['MBSAB_BILL_AMOUNT'],"duedate"=>$res['MBSAB_BILL_DUE_DATE'],"billstatus"=>$res['MBSAB_BILL_PAID']);
        }
        else{
            return null;
        }
 ...
 * @return boolean
 */
function AddTransaction($partnerId, $id, $type, $source, $dest, $amount, $fees, $datetime, $result_code, $result_desc,$dbConnexionID) {
    $sqlQuery = "SELECT * FROM tb_transactions WHERE col_id = '".$id."'";
    try {
        $req = mysqli_query($dbConnexionID, $sqlQuery);
        if (mysqli_num_rows($req) == 0) {
            $sqlQuery = "INSERT INTO tb_transactions(col_id,col_partner_id,col_type,col_source,col_destination,col_amount,col_fees,col_datetime,col_result_code,col_result_desc) VALUES ('" . $id . "','" . $partnerId . "','" . $type . "','" . $source . "','" . $dest . "','" . $amount . "','" . $fees . "','" . $datetime . "','" . $result_code . "','" . $result_desc . "')";
            mysqli_query($dbConnexionID, $sqlQuery);
            return TRUE;
        } else {
            return FALSE;

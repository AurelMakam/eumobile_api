...
//                    récuperation du timeout du partenaire
                        ...
                        if ($timeout == 0) {
                            $informix = null;
                            unset($informix);
                            AddTransaction($partnerId, $transaction,$serviceId,"PAYMENT REQUEST", $partnerCode,$phone, $amount, $intitrxdetails["fee"], $intitrxdetails["tax"], $intitrxdetails["date"],$intitrxdetails["result_code"],$intitrxdetails["result_desc"],$transaction,1, $connection);
                            ...
                            while ($statutr != 1 && $inc <= $timeout) {
                                ...
                                if ($statutr == 1) {
                                    ...
                                    AddTransaction($partnerId, $payment_transaction,$serviceId,"PAYMENT REQUEST AUTHORIZATION", $phone,$partnerCode, $amount, $transdetails["fee"], $transdetails["tax"], $transdetails["date"],$transdetails["result_code"],$transdetails["result_desc"],$transaction,0, $connection);
                                    ...
                                    $informix = null;
                                    unset($informix);
                                    echo json_encode(array("statut" => 100, "message" => $langFront["Label"][12], "phone" => $tab[0], "amount" => $tab[2], "reference" => $tab[3], "transaction" => $payment_transaction, "balance"=>$subsdetails["balance"]));
                                    exit();
                                } else {
                                    $inc++;
